...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Storage Key
    const STORAGE_KEY = 'EXIMIUS_RUNNING_LOG_DATA_V1';
    ...
    function initApp() {
        ...
        // If no rows saved, initialize with default empty rows (10)
        if (!logData.rows || logData.rows.length === 0) {
            createEmptyRows(parseInt(defaultRowsSelect.value) || 10);
        } else {
            renderTableRows();
        }
        ...
        logData.rows.forEach((row, index) => {
            ...
            tr.innerHTML = `
                ...
                <td class="col-approved">
                    <input type="text" class="cell-input text-center cell-approved" value="${escapeHtml(row.approvedBy)}" placeholder="Manager / Sig">
                </td>
    ...
    function fillSampleData() {
        logData.name = 'K. A. Perera';
        logData.department = 'Engineering & Service';
        logData.month = getCurrentMonthString();
        logData.rate = 27.00;
        logData.defaultApproved = 'M. Silva';
        logData.showViaColumn = true;
        updateViaColumnVisibility();

        logData.rows = [
            { id: generateId(), date: '2026-10-02', startLoc: 'Head Office - Colombo', viaLoc: '', endLoc: 'Biyagama Factory', distance: 28.5, details: 'Solar Inverter Inspection Visit', approvedBy: 'M. Silva' },
            { id: generateId(), date: '2026-10-05', startLoc: 'Biyagama Factory', viaLoc: 'Kelaniya Site', endLoc: 'Head Office - Colombo', distance: 34.0, details: 'Return travel after testing & site inspection', approvedBy: 'M. Silva' },
            { id: generateId(), date: '2026-10-08', startLoc: 'Head Office', viaLoc: 'Kahawatta Substation', endLoc: 'Horana Site', distance: 58.0, details: '5kW Hybrid System Commissioning & Inspection', approvedBy: 'M. Silva' },
            { id: generateId(), date: '2026-10-08', startLoc: 'Horana Site', viaLoc: '', endLoc: 'Head Office', distance: 45.0, details: 'Return trip to Colombo office', approvedBy: 'M. Silva' },
            { id: generateId(), date: '2026-10-12', startLoc: 'Head Office', viaLoc: 'Negombo Site', endLoc: 'Katunayake EPZ', distance: 42.0, details: 'Client Progress Meeting & Survey', approvedBy: 'M. Silva' },
            { id: generateId(), date: '2026-10-15', startLoc: 'Head Office', viaLoc: 'Pallekele EPZ', endLoc: 'Kandy Project Site', distance: 125.0, details: 'Substation Control Panel Maintenance', approvedBy: 'M. Silva' },
            { id: generateId(), date: '2026-10-16', startLoc: 'Kandy Project Site', viaLoc: '', endLoc: 'Head Office', distance: 115.0, details: 'Return travel from Kandy Site', approvedBy: 'M. Silva' }
        ];

        // Pad with empty rows to make 10 total
        while (logData.rows.length < 10) {
            logData.rows.push({
                id: generateId(),
                date: '',
                startLoc: '',
